...
import { validate, summary, hasErrors, BIND_KEYS, HOME_NAV_OPTIONS, SEV_ERROR, SEV_WARN, SEV_INFO } from './validation.js?v=2.19.1';
...
class BMSPanelEditor extends HTMLElement {
  ...
  _extractConfig(attrs) {
    return {
      ...
      airplay_name:   attrs.airplay_name || '',
  ...
  _renderOverview(panel, issues) {
    ...
    return `
      ...
      <div class="card">
        ...
        ${cfg.airplay_enabled ? `
        <div class="field-row">
          <label>Имя в AirPlay</label>
          <input type="text" id="airplay-name" class="control" maxlength="40"
                 placeholder="BMS ${esc(panel.panel_name || panel.panel_id)}"
                 value="${esc(cfg.airplay_name || '')}">
        </div>` : ''}
        <div class="hint" style="margin: -4px 0 10px;">
          Панель появится в списке AirPlay на iPhone и Mac как
          «${esc(cfg.airplay_name || ('BMS ' + (panel.panel_name || panel.panel_id)))}».
  ...
  _wireTabEvents(panel, issues) {
    ...
    const apName = $('#airplay-name');
    if (apName) apName.oninput = e => { cfg.airplay_name = e.target.value.slice(0, 40); this._markDirty(); };
    const lang = $('#lang');
